...
export default {
     ...
     * 获取 - 可用余额并且累计收入
     */
    queryAgentAccount: self => apibasics.get(`${config.url.origin}/ycpd/cas/queryAgentAccount?token=${window.localStorage.getItem('ycpd_token')}&agentInfoId=${window.localStorage.getItem('ycpd_agentInfoId')}`, '可用余额', self),
